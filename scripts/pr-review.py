#!/usr/bin/env python3
"""pr-review — automate GitHub PR review triage using herdr worktrees + the pi agent.

Workflow:
  * discover your review-requested PRs (drops drafts + ones you already approved)
  * rank them by cost-to-review (size + risk heuristic) into buckets
  * spin up an isolated herdr workspace + git worktree + pi agent per PR
  * check status / read results / tear everything down cleanly

Subcommands:
  list                       ranked table of pending reviews
  start <selector...>        fan out reviews (buckets: high|substantial|low|trivial|all, or repo!num)
  status                     show agent status for active reviews
  read <repo!num>            dump the agent output for a review
  close <selector...|--all|--idle>   tear down workspace + worktree + branch

Relies on: gh, herdr, git, and ~/dev/ai-kit/scripts/run-agent.sh
"""
...
# Config
# ---------------------------------------------------------------------------
REPO_ROOT = Path(os.environ.get("PR_REVIEW_REPO_ROOT", Path.home() / "indigo"))
RUN_AGENT = Path(os.environ.get(
    "PR_REVIEW_RUN_AGENT",
    Path.home() / "dev/ai-kit/scripts/run-agent.sh",
))
PROFILE = os.environ.get("PR_REVIEW_PROFILE", "indigo")
CACHE_DIR = Path(os.environ.get("PR_REVIEW_CACHE", Path.home() / ".cache/pr-review"))
STATE_FILE = CACHE_DIR / "state.json"
CTX_FILE = CACHE_DIR / "context.md"
RANK_CACHE = CACHE_DIR / "ranked.json"
DEFAULT_MAX = int(os.environ.get("PR_REVIEW_MAX", "4"))

BUCKETS = ["high", "substantial", "low", "trivial"]
BUCKET_EMOJI = {"high": "🔴", "substantial": "🟠", "low": "🟡", "trivial": "🟢"}

# risk keywords bump a PR up a bucket
RISK_KEYWORDS = (
    "auth", "security", "secret", "token", "migrat", "schema", "amplify",
    "cdk", "prompt", "phi", "encrypt", "permission", "audit", "billing",
)
...
def gh_api(path: str, check: bool = True):
    return run_json(["gh", "api", path], check=check)


def gh_api_all(path: str, per_page: int = 100) -> list:
    """Fetch every page of a list endpoint (gh --paginate emits one JSON
    array per page, which isn't parseable as a single document)."""
    sep = "&" if "?" in path else "?"
    items: list = []
    page = 1
    while True:
        chunk = gh_api(f"{path}{sep}per_page={per_page}&page={page}", check=False)
...
# ---------------------------------------------------------------------------
# State (pr key -> {workspace_id, tab_id, path, branch, agent, repo, num, url})
# ---------------------------------------------------------------------------
def load_state() -> dict:
...
def fetch_pending(login: str) -> list[dict]:
    """Open, review-requested PRs, excluding drafts and ones I've approved."""
    prs = run_json([
        "gh", "search", "prs",
        "--review-requested", "@me",
        "--state", "open",
    ...
    for m in prs:
        ...
        num = m["number"]
        reviews = gh_api_all(f"repos/{nwo}/pulls/{num}/reviews")
        approved = any(
            r.get("state") == "APPROVED"
            and (r.get("user") or {}).get("login") == login
            for r in reviews
        )
        if approved:
            continue
...
def diff_stats(pr: dict) -> dict:
    files = gh_api_all(f"repos/{pr['nwo']}/pulls/{pr['num']}/files")
    ...
    for c in files:
        ...
        if status == "added":
            ...
            deleted += 1
    return {"files": len(files), "add": add, "del": dele,
            "new": new, "deleted": deleted}


def bucket_for(pr: dict, stats: dict) -> str:
...
def rank(pending: list[dict]) -> list[dict]:
    ...
    for pr in pending:
        stats = diff_stats(pr)
        pr = {**pr, "stats": stats, "bucket": bucket_for(pr, stats)}
        ranked.append(pr)
...
def load_ranked(refresh: bool = False) -> list[dict]:
    ...
    return rank(fetch_pending(my_login()))


def key(pr: dict) -> str:
    return f"{pr['repo']}!{pr['num']}"


def pr_from_url(url: str) -> dict:
    """Parse a GitHub PR URL into a pr dict (repo, num, url, nwo)."""
    url = url.strip().rstrip("/")
    if "/pull/" not in url:
        raise SystemExit(f"not a PR url: {url}")
    left, num = url.split("/pull/", 1)
    num = int(num.split("/")[0].split("?")[0])
    nwo = left.split("github.com/", 1)[-1]
    repo = nwo.split("/")[-1]
    return {"nwo": nwo, "repo": repo, "num": num, "url": url}


def resolve_key(state: dict, selector: str) -> str | None:
    """Accept a repo!num key or a full PR url, return the state key."""
    if selector in state:
        return selector
    if "/pull/" in selector:
        k = key(pr_from_url(selector))
...
def select(ranked: list[dict], selectors: list[str]) -> list[dict]:
    ...
    for pr in ranked:
        if pr["bucket"] in want_buckets or key(pr) in want_keys:
            chosen.append(pr)
    return chosen
...
def spin_up(pr: dict, state: dict) -> None:
    k = key(pr)
    if k in state:
        print(f"  skip {k}: already active (workspace {state[k]['workspace_id']})")
        return
    repo, num, url = pr["repo"], pr["num"], pr["url"]
    d = repo_dir(repo)
    if not d.exists():
    ...
    wt_path = Path.home() / ".herdr/worktrees" / repo / f"review-pr-{num}"
...
def cmd_list(args):
    ...
    for pr in ranked:
        ...
        s = pr["stats"]
        active = " [active]" if key(pr) in state else ""
        print(f"  {key(pr):32} {s['files']:>3}f +{s['add']}/-{s['del']}"
              f"  {pr['title'][:60]}{active}")
    print()
...
def cmd_start(args):
    ...
    for pr in chosen:
        print(f"  {BUCKET_EMOJI[pr['bucket']]} {key(pr)}  {pr['title'][:60]}")
...
def cmd_read(args):
    state = load_state()
    k = resolve_key(state, args.pr)
    e = state.get(k) if k else None
    if not e:
        raise SystemExit(f"no active review for {args.pr}")
...
def main():
    ...
    pl.add_argument("--refresh", action="store_true", help="re-query GitHub")
    pl.set_defaults(func=cmd_list)

    ps = sub.add_parser("start", help="fan out reviews")
    ps.add_argument("selectors", nargs="*",
                    help="buckets (high|substantial|low|trivial|all) or repo!num")
    ...
    po = sub.add_parser("open", help="set up review(s) from PR url(s)")
    po.add_argument("urls", nargs="+", help="one or more GitHub PR urls")
